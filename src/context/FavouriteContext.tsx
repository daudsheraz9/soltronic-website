...
export function FavouriteProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load initial favourites from localStorage & sync with Supabase user profile
  useEffect(() => {
    try {
      ...
      // If user is authenticated, sync/merge with Supabase
      if (user) {
  ...
  const syncSupabaseFavourites = async (userId: string, localItems: FavouriteItem[]) => {
    try {
      // Upsert profile in Supabase profiles table if needed
      ...
      // Save user favourites in user_metadata or Supabase favourites
  ...
  const addToFavourite = (product: Product, quantity: number = 1) => {
    ...
    openDrawer(); // Pop open slide-over drawer for instant UX!
  };
