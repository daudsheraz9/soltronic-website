...
import ProductSpecificationDisplay from '@/components/product/ProductSpecificationDisplay';
            ...
            >
              ...
              {isZooming ? (
                ...
                <img 
                  src={safeUrl} 
                  alt={product.title} 
                  className="max-h-full max-w-full object-contain p-4 pointer-events-none transition-transform duration-200" 
                />
              )}
              
              <div className="absolute bottom-3 right-3 bg-white/95 text-gray-800 backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-semibold shadow-md flex items-center gap-1.5 pointer-events-none border border-gray-200">
                <i className="fa-solid fa-magnifying-glass text-xs text-[#107022]"></i>
                <span className="text-gray-800 font-semibold">{isZooming ? (zoomMode === 'standard' ? '1.5x Zoom' : '2.1x Pro Zoom') : 'Hover to Zoom'}</span>
              </div>
            </div>
